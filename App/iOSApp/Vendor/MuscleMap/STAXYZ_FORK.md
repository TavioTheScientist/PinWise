...
> **Renamed 2026-08-01: PinWise → Staxyz.** This file was `PINWISE_FORK.md`; it and the 5 Swift
> comments citing it (`BodyMapView.swift`, `BodyRenderer.swift`, `MuscleRegion.swift`,
> `BodyView.swift` ×2) were renamed together.
>
> Also relevant to the Android plan: this fork is **SwiftUI + CoreGraphics**. A Flutter build
> cannot reuse any of it — the region-precise injection map (~4.8k lines) has to be rebuilt.
...
Vendored into the app target rather than consumed as a SwiftPM dependency because Staxyz needs one
...
All Staxyz GitHub work lives on `TavioTheScientist`, but the local `gh` CLI is authenticated as
...
injection map, where Staxyz tracks 16 distinct sites split left/right and upper/lower, and the
...
which is exactly why a clip-mask approach in Staxyz was rejected.
...
the image's left is therefore the subject's **right**. Callers must flip for front views; Staxyz does
...
`BodyView` renders, and the one Staxyz uses. The animated / pulse / zoomable variants and the
interactive, tooltip and accessibility overlays are untouched: they wrap `BodyRenderer` through their
own initializers, and the overlays handle hit-testing and labels rather than fill. If Staxyz ever
