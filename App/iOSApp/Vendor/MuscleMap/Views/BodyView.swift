...
extension BodyView {
    ...
    public func regionHeatmap(_ data: [RegionIntensity],
        ...
        let scale = HeatmapColorScale(colors: colorScale.colors, interpolation: config.interpolation)
        for entry in data {
            if let threshold = config.threshold, entry.intensity < threshold { continue }
            let color = entry.color ?? scale.color(for: entry.intensity)
            copy.regionHighlights[entry.region] = MuscleHighlight(
                muscle: entry.region.muscle, color: color, opacity: 1.0
            )
        }
        return copy
    }
