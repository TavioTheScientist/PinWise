...
enum DebugSeeder {
    ...
    static func seedIfRequested() async {
        ...
        // SIMULATOR ONLY, and this is not belt-and-braces on top of `#if DEBUG` — it closes a
        // different hole. `#if DEBUG` keeps the seeder out of shipping binaries; this keeps it out of
        // a real person's Health record. Part B writes ~2.5k samples to the SYSTEM HealthKit store,
        // which outlives the app: deleting Staxyz does not remove them, and there is no bulk undo —
        // the owner would be deleting fabricated weight, sleep and heart-rate rows by hand in the
        // Health app. A developer running a Debug build on their own iPhone is one env var away from
        // that, and "they set the variable deliberately" is not consent to pollute a medical record.
        // Screenshots are captured on the simulator, so this costs the feature nothing.
        log("Refusing to seed: this is a real device, and the seeder writes fabricated samples into "
            + "the system Health store, which survives app deletion and has no bulk undo. "
            + "Simulator only.")
        return
        #endif
    ...
    /// Weekly semaglutide slots: day 0, 7, … 119. Day 119 IS today, so the seeding day is always a
    /// scheduled day and the protocol never looks dormant in a screenshot.
    ///
    /// Note what this does NOT mean: today's dose is **logged**, not left due, because the log loop
    /// below skips only `semaMissedSlots` and `semaSkippedSlot`. So the seeded state is "fully caught
    /// up", and Home shows a next-dose pin rather than the due-dose CTA. To photograph the due state
    /// instead, add `windowDays - 1` to `semaMissedSlots`, bump `markerKey`, and re-seed.
    ///
    /// **The timeline is anchored to the day the seeder RUNS and then ages.** Nothing re-anchors it, so
    /// a week later "today's" slot is a week old and the weekday below is last week's. That is fine for
    /// a screenshot session and wrong for anything long-lived — re-seed rather than reasoning about
    /// drift.
    ...
    private static func seedStore(context: ModelContext) -> Bool {
        ...
        let sema = SavedProtocol(
            ...
            startDate: day(0),
            // The weekday is DERIVED, never written out. The schedule's weekday is whatever day(0)
            // falls on — i.e. the weekday the seeder happened to run — so a hardcoded "Sunday-morning
            // shot" was wrong on six days out of seven, and visibly so: the protocol detail shows this
            // note directly under a "Weekly · Mon" cadence row. A note that contradicts the schedule
            // beside it is exactly the detail that makes a screenshot look fabricated.
            notes: "Titrating per label. \(weekdayName(of: day(0)))-morning shot, 8 am.",
        ...
        do {
            ...
            log("Part A: \(rows) SwiftData rows — 2 protocols, 2 vials, "
                ...
                // Counted, not written out: a hardcoded "18" silently becomes a lie the first time
                // `windowDays` or the weigh-in cadence changes, and a summary line nobody can trust
                // is worse than no summary.
                + "\(weightRows) weight entries, \(windowDays) health snapshots.")
            return true
        } catch {
    ...
    /// Full weekday name for the note above. `.wide` rather than a hardcoded English string so the
    /// note matches the cadence row the app draws beside it, which is itself locale-formatted.
    private static func weekdayName(of date: Date) -> String {
