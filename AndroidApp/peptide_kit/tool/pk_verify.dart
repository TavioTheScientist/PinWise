...
void main() {
  ...
  section('Reconstitution record (stability Phase 0)');
  {
    ...
    check(
      const ReconstitutionRecord().isEmpty,
      'a blank record reports itself empty',
    );
    check(
    ...
    check(
      ReconstitutionTimeline.sentence(
            const ReconstitutionRecord(),
            asOf: now,
          ) ==
          null,
    ...
    check(
      ReconstitutionTimeline.clauses(
            ReconstitutionRecord(reconstitutedOn: d(2026, 8, 4)),
            asOf: now,
          ).first ==
          'Reconstituted today',
    ...
    check(
      ReconstitutionTimeline.clauses(
            ReconstitutionRecord(reconstitutedOn: d(2026, 8, 3)),
            asOf: now,
          ).first ==
    ...
    check(
      ReconstitutionTimeline.clauses(
            ReconstitutionRecord(reconstitutedOn: d(2026, 8, 9)),
            asOf: now,
          ).first ==
          'Reconstituted',
    ...
    final many = ReconstitutionRecord(
      excursions: [
        ...
        StorageExcursion(
          ...
          date: d(2026, 8, 1),
          hours: 48,
          exposedTo: VialStorage.roomTemperature,
        ),
      ],
    );
    check(
    ...
    check(
      many.totalExcursionHours == 55.5,
      'totalExcursionHours sums what was reported',
    );
    check(
    ...
    check(
      StorageExcursion(id: 'x', date: now, hours: 1.5).durationPhrase ==
          '1.5-hour',
      'fractional hours keep their precision',
    );
    check(
      StorageExcursion(id: 'x', date: now, hours: -3).hours == 0,
    ...
    check(
      Diluent.bacteriostaticWater.isPreserved &&
          !Diluent.sterileWater.isPreserved,
    ...
    final everyClause =
        (ReconstitutionTimeline.clauses(full, asOf: now) +
